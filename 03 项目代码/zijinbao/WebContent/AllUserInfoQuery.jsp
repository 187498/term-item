						...
						<form class="form-horizontal" action="http://localhost:8080/zijinbao/query_user">
							<div class="col-md-4">
									<div class="form-group">
					                    ...
					                    <div class="col-sm-9">
					                    	<section>
					                        	<input type="text" class="form-control" id="inputEmail3" name="u_name" value="${name }" placeholder="姓名">
					                        </section>
					                    </div>
					                </div>
							</div>
							<div class="col-md-4">
					                <div class="form-group">
					                    ...
					                    <div class="col-sm-9">
					                    	<section>
					                        	<input type="text" class="form-control" id="inputEmail3" name="u_account" value="${account }" placeholder="账户">
					                        </section>
					                    </div>
					                </div>
					                <div class="form-group">
							...
							<div class="col-md-4">
									<div class="form-group">
					                    ...
					                    <div class="col-sm-9">
					                    	<section>
					                        	<input type="text" class="form-control" id="inputEmail3" name="u_id" value="${id }" placeholder="身份证">
					                        </section>
					                    </div>
					                </div>
        ...
        <!--分页-->
            ...
            <c:if test="${requestScope.pageBean.pageNum == 1 && requestScope.pageBean.totalPage == 1}">
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">
                    ...
                    <c:if test="${requestScope.pageBean.pageNum != i}">
                        <a href="http://localhost:8080/zijinbao/query_user?pageNum=${i}&u_name=${name}&u_account=${account}&u_id=${id}">${i}</a>                                        
                    </c:if>                        
                </c:forEach>                    
            </c:if>
            <%--如果当前页为第一页时，就没有上一页这个超链接显示 --%>
            <c:if test="${requestScope.pageBean.pageNum == 1 && requestScope.pageBean.totalPage != 1 && requestScope.pageBean.totalPage != 0}">
            	<a href="http://localhost:8080/zijinbao/query_user?pageNum=1&u_name=${name}&u_account=${account}&u_id=${id}">首页</a>
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">
                    <c:if test="${requestScope.pageBean.pageNum == i}">
                        ${i}
                    </c:if>                
                ...
                <a href="http://localhost:8080/zijinbao/query_user?pageNum=${requestScope.pageBean.pageNum+1}&u_name=${name}&u_account=${account}&u_id=${id}">下一页</a> 
                <a href="http://localhost:8080/zijinbao/query_user?pageNum=${requestScope.pageBean.totalPage}&u_name=${name}&u_account=${account}&u_id=${id}">尾页</a>                   
            </c:if>
            
            <%--如果当前页不是第一页也不是最后一页，则有上一页和下一页这个超链接显示 --%>
            <c:if test="${requestScope.pageBean.pageNum > 1 && requestScope.pageBean.pageNum < requestScope.pageBean.totalPage}">
            	<a href="http://localhost:8080/zijinbao/query_user?pageNum=1&u_name=${name}&u_account=${account}&u_id=${id}">首页</a>
                <a href="http://localhost:8080/zijinbao/query_user?pageNum=${requestScope.pageBean.pageNum-1}&u_name=${name}&u_account=${account}&u_id=${id}">上一页</a>
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">    
                    <c:if test="${requestScope.pageBean.pageNum == i}">
                        ${i}
                    </c:if>            
