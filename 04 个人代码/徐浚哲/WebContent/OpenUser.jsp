 ...
 <nav class="navbar navbar-default" style="height: 70px; border: #000000">
  ...
  <div class="container" style="positon:relative; top: 1000px;">
        ...
        <!--分页-->
            ...
            <br/>
            <a href="http://localhost:8080/zijinbao/uf_user?pageNum=1">首页</a>
            ...
            <c:if test="${requestScope.pageBean.pageNum == 1}">
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">
                    ...
                    <c:if test="${requestScope.pageBean.pageNum != i}">
                        <a href="http://localhost:8080/zijinbao/uf_user?pageNum=${i}">${i}</a>                                        
                    </c:if>                        
                </c:forEach>
                <a href="http://localhost:8080/zijinbao/uf_user?pageNum=${requestScope.pageBean.pageNum+1}">下一页</a>                    
            </c:if>
            
            <%--如果当前页不是第一页也不是最后一页，则有上一页和下一页这个超链接显示 --%>
            <c:if test="${requestScope.pageBean.pageNum > 1 && requestScope.pageBean.pageNum < requestScope.pageBean.totalPage}">
                <a href="http://localhost:8080/zijinbao/uf_user?pageNum=${requestScope.pageBean.pageNum-1}">上一页</a>
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">    
                    <c:if test="${requestScope.pageBean.pageNum == i}">
                        ${i}
                    </c:if>            
            ...
            <%--尾页 --%>
            <a href="http://localhost:8080/zijinbao/uf_user?pageNum=${requestScope.pageBean.totalPage}">尾页</a>
           		</div>
           		<div class="col-md-4"></div>
           	</div>
         </div>
