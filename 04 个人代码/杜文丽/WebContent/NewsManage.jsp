    ...
    <!--左侧导航栏-->
 	<div id="wrapper">
					...
					<div class="col-md-4">
						<form class="form-horizontal" action="http://localhost:8080/zijinbao/n_query">
					                <div class="form-group">
					                    ...
					                    <div class="col-sm-9">
					                        <input type="text" class="form-control" id="inputEmail3" name="n_title" placeholder="新闻标题">
					                    </div>
					                </div>
					                <div class="form-group">
        ...
        <div class="container">
            ...
            <br/>
            <a href="http://localhost:8080/zijinbao/NewsMPage?pageNum=1">首页</a>
            ...
            <c:if test="${requestScope.pageBean.pageNum == 1}">
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">
                    ...
                    <c:if test="${requestScope.pageBean.pageNum != i}">
                        <a href="http://localhost:8080/zijinbao/NewsMPage?pageNum=${i}">${i}</a>                                        
                    </c:if>                        
                </c:forEach>
                <a href="http://localhost:8080/zijinbao/NewsMPage?pageNum=${requestScope.pageBean.pageNum+1}">下一页</a>                    
            </c:if>
            
            <%--如果当前页不是第一页也不是最后一页，则有上一页和下一页这个超链接显示 --%>
            <c:if test="${requestScope.pageBean.pageNum > 1 && requestScope.pageBean.pageNum < requestScope.pageBean.totalPage}">
                <a href="http://localhost:8080/zijinbao/NewsMPage?pageNum=${requestScope.pageBean.pageNum-1}">上一页</a>
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">    
                    <c:if test="${requestScope.pageBean.pageNum == i}">
                        ${i}
                    </c:if>            
            ...
            <%--尾页 --%>
            <a href="http://localhost:8080/zijinbao/NewsMPage?pageNum=${requestScope.pageBean.totalPage}">尾页</a>
		</div>
           		<div class="col-md-4"></div>
           	</div>
         </div>
			<br>
	 	     ...
	 	     <div class="row">
				...
				<div class="col-md-4">
					<div class="col-sm-offset-2 col-sm-10">
					    <a href="AddNews.jsp" class="btn btn-primary btn-lg btn-block">添加新闻</a>
					</div>	 
	 			</div>
	 			<div class="col-md-4"></div>
	 		 </div>
